...
BlockController.prototype.transformBlock = function(block, info) {
  ...
  if(block instanceof bcoin.Block) {
      merkleRoot = bcoin.utils.util.revHex(merkleRoot)
  }
  ...
  return {
    ...
    reward: this.getBlockReward(block),
    chainReward: this.getChainReward(info.height),
    isMainChain: true,
...
/**
 * Get block reward without fees
 * Calculated by formula, snippet from v0.4.3
 *
 * @param height {Number}
 * @returns {Number}
 */
BlockController.prototype.getChainReward = function(height) {
    ...
    var subsidy = new bitcore.crypto.BN(50 * 1e8);
    subsidy = subsidy.shrn(halvings);

    var resultSatoshis = parseInt(subsidy.toString(10));

    return bitcore.Unit.fromSatoshis(resultSatoshis).toBTC();
};

/**
 * Gets actual block reward that was specified in the generation transaction
 * @param block {Block}
 * @returns {Number}
 */
BlockController.prototype.getBlockReward = function(block) {
  function blockRewardReducer(accumulator, output) {
      return accumulator + output.value;
  }

  var generationTX = block.txs[0];

  var rewardAmount = generationTX.outputs.reduce(blockRewardReducer, 0);

  return bitcore.Unit.fromSatoshis(rewardAmount).toBTC();
};
