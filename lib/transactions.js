...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  if (address) {
    ...
    transformed.addr = this.common.translateOutputAddress(address.toString(this._network));
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (address) {
    address.network = this._network;
    transformed.scriptPubKey.addresses = [this.common.translateOutputAddress(address.toString(this._network))];
...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  for (var i = 0; i < transaction.outputs.length; i++) {
    ...
    if (output.script) {
      ...
      address.network = this._network;
      address = self.common.translateOutputAddress(address.toString(this._network));
